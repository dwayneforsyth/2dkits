...
static uint8_t adv_data[] = {
    ...
    0x03, 0x02, 0xf0, 0xff, // service 0xfff0
    0x0E, 0xFF, 0x98, 0x5A, 0xFF, 0x00, 0x00, 0x01,
                0x94, 0x96, 0x10, 0x00, 0x20, 0x01, 0x44,
//    0x05, 0x12, 0x0D, 0x00, 0x0D, 0x00, //Peripheral connection interval randge
//    0x02, 0x0A, 0x07, // power level
    0x08, 0x09, 'C',  'o',  'o',  'l',  'L',  'E',  'D'// ,  'x' // local name
};
    rc = ble_gap_adv_set_data(adv_data, sizeof(adv_data));
    if (rc != 0) {
        MODLOG_DFLT(ERROR, "error setting advertisement; rc=%d %d\n", rc, BLE_HS_EROLE);
        return;
    }
...
app_main(void)
{
    ...
    uint8_t mac_addr[6] = {0xff, 0x00, 0x00, 0x01, 0x94, 0x98};
    esp_iface_mac_addr_set(mac_addr, ESP_MAC_BT);
    ...
    rc = ble_svc_gap_device_name_set("CoolLEDX");
    assert(rc == 0);
