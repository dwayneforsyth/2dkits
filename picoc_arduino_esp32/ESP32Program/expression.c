...
{
    printf((char *)"Expression stack [0x%lx,0x%lx]: ", (long)HeapStackTop, (long)StackTop);
    ...
    while (StackTop != NULL)
    {
        ...
        { 
            ...
            {
                case TypeVoid:      printf((char *)"void"); break;
                case TypeInt:       printf((char *)"%d:int", StackTop->Val->Val->Integer); break;
                case TypeShort:     printf((char *)"%d:short", StackTop->Val->Val->ShortInteger); break;
                case TypeChar:      printf((char *)"%d:char", StackTop->Val->Val->Character); break;
                case TypeLong:      printf((char *)"%d:long", StackTop->Val->Val->LongInteger); break;
                case TypeUnsignedShort: printf((char *)"%d:unsigned short", StackTop->Val->Val->UnsignedShortInteger); break;
                case TypeUnsignedInt: printf((char *)"%d:unsigned int", StackTop->Val->Val->UnsignedInteger); break;
                case TypeUnsignedLong: printf((char *)"%d:unsigned long", StackTop->Val->Val->UnsignedLongInteger); break;
                case TypeFP:        printf((char *)"%f:fp", StackTop->Val->Val->FP); break;
                case TypeFunction:  printf((char *)"%s:function", StackTop->Val->Val->Identifier); break;
                case TypeMacro:     printf((char *)"%s:macro", StackTop->Val->Val->Identifier); break;
                case TypePointer:
                    if (StackTop->Val->Val->Pointer == NULL)
                        printf((char *)"ptr(NULL)");
                    else if (StackTop->Val->Typ->FromType->Base == TypeChar)
                        printf((char *)"\"%s\":string", (char *)StackTop->Val->Val->Pointer);
                    else
                        printf((char *)"ptr(0x%lx)", (long)StackTop->Val->Val->Pointer); 
                    break;
                case TypeArray:     printf((char *)"array"); break;
                case TypeStruct:    printf((char *)"%s:struct", StackTop->Val->Val->Identifier); break;
                case TypeUnion:     printf((char *)"%s:union", StackTop->Val->Val->Identifier); break;
                case TypeEnum:      printf((char *)"%s:enum", StackTop->Val->Val->Identifier); break;
                case Type_Type:     PrintType(StackTop->Val->Val->Typ); printf((char *)":type"); break;
                default:            printf((char *)"unknown"); break;
            }
            printf((char *)"[0x%lx,0x%lx]", (long)StackTop, (long)StackTop->Val);
        }
        else
        { 
            /* it's an operator */
            printf((char *)"op='%s' %s %d", OperatorPrecedence[(int)StackTop->Op].Name, 
                (StackTop->Order == OrderPrefix) ? "prefix" : ((StackTop->Order == OrderPostfix) ? "postfix" : "infix"), 
                StackTop->Precedence);
            printf((char *)"[0x%lx]", (long)StackTop);
        }
...
long ExpressionAssignInt(struct ParseState *Parser, struct Value *DestValue, long FromInt, int After)
{
    ...
    if (!DestValue->IsLValue) 
        ProgramFail(Parser,(char *)"can't assign to this"); 
...
void ExpressionStackPushDereference(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *DereferenceValue)
{
    ...
    if (DerefDataLoc == NULL)
        ProgramFail(Parser,(char *)"NULL pointer dereference");
...
void ExpressionAssignToPointer(struct ParseState *Parser, struct Value *ToValue, struct Value *FromValue, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    else if (AllowPointerCoercion && FromValue->Typ->Base == TypePointer)
    {
        ...
        AssignFail(Parser,(char *)"%t from %t", ToValue->Typ, FromValue->Typ, 0, 0, FuncName, ParamNo); 
}
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    if (!DestValue->IsLValue && !Force) 
        AssignFail(Parser,(char *)"not an lvalue", NULL, NULL, 0, 0, FuncName, ParamNo); 

    if (IS_NUMERIC_COERCIBLE(DestValue) && !IS_NUMERIC_COERCIBLE_PLUS_POINTERS(SourceValue, AllowPointerCoercion))
        AssignFail(Parser,(char *)"%t from %t", DestValue->Typ, SourceValue->Typ, 0, 0, FuncName, ParamNo); 
    ...
    switch (DestValue->Typ->Base)
    {
        ...
        case TypeArray:
            ...
            if (DestValue->Typ->ArraySize != SourceValue->Typ->ArraySize)
                AssignFail(Parser,(char *)"from an array of size %d to one of size %d", NULL, NULL, DestValue->Typ->ArraySize, SourceValue->Typ->ArraySize, FuncName, ParamNo);
        ...
        default:
            AssignFail(Parser,(char *)"%t", DestValue->Typ, NULL, 0, 0, FuncName, ParamNo); 
            break;
    }
}
...
void ExpressionQuestionMarkOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, struct Value *BottomValue, struct Value *TopValue)
{
    if (!IS_NUMERIC_COERCIBLE(TopValue))
        ProgramFail(Parser,(char *)"first argument to '?' should be a number");
    ...
    switch (Op)
    {
        case TokenAmpersand:
            if (!TopValue->IsLValue)
                ProgramFail(Parser,(char *)"can't get the address of this");
        ...
        default:
            ...
            if (TopValue->Typ == &FPType)
            {
                ...
                switch (Op)
                {
                    ...
                    default:                ProgramFail(Parser,(char *)"invalid operation"); break;
                }
            ...
            else if (TopValue->Typ->Base == TypePointer)
            {
                ...
                if (TopValue->Val->Pointer == NULL)
                    ProgramFail(Parser,(char *)"invalid use of a NULL pointer");
                ...
                ProgramFail(Parser,(char *)"invalid operation");
            break;
    }
}
...
void ExpressionPostfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *TopValue)
{
    ...
    if (IS_NUMERIC_COERCIBLE(TopValue))
    {
        ...
        switch (Op)
        {
            ...
            case TokenRightSquareBracket:   ProgramFail(Parser,(char *)"not supported"); break;  /* XXX */
            case TokenCloseBracket:         ProgramFail(Parser,(char *)"not supported"); break;  /* XXX */
            default:                        ProgramFail(Parser,(char *)"invalid operation"); break;
        }
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
    ...
    if (BottomValue == NULL || TopValue == NULL)
        ProgramFail(Parser,(char *)"invalid expression");
    ...
    { 
        ...
        if (!IS_NUMERIC_COERCIBLE(TopValue))
            ProgramFail(Parser,(char *)"array index must be an integer");
        ...
        switch (BottomValue->Typ->Base)
        {
            ...
            default:          ProgramFail(Parser,(char *)"this %t is not an array", BottomValue->Typ);
        }
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
    ...
    if (LexGetToken(Parser, &Ident, TRUE) != TokenIdentifier)
        ProgramFail(Parser,(char *)"need an structure or union member after '%s'", (Token == TokenDot) ? "." : "->");
    ...
    { 
        ...
        if (StructType->Base != TypeStruct && StructType->Base != TypeUnion)
            ProgramFail(Parser,(char *)"can't use '%s' on something that's not a struct or union %s : it's a %t", (Token == TokenDot) ? "." : "->", (Token == TokenArrow) ? "pointer" : "", ParamVal->Typ);
            
        if (!TableGet(StructType->Members, Ident->Val->Identifier, &MemberValue, NULL, NULL, NULL))
            ProgramFail(Parser,(char *)"doesn't have a member called '%s'", Ident->Val->Identifier);
...
int ExpressionParse(struct ParseState *Parser, struct Value **Result)
{
    ...
    do
    {
        ...
        { 
            ...
            { 
                ...
                if (OperatorPrecedence[(int)Token].PrefixPrecedence == 0)
                    ProgramFail(Parser,(char *)"operator not expected here");
                ...
                { 
                    ...
                    if (IS_TYPE_TOKEN(BracketToken) && (StackTop == NULL || StackTop->Op != TokenSizeof) )
                    {
                        ...
                        if (LexGetToken(Parser, &LexValue, TRUE) != TokenCloseBracket)
                            ProgramFail(Parser,(char *)"brackets not closed");
        ...
        { 
            ...
            if (!PrefixState)
                ProgramFail(Parser,(char *)"identifier not expected here");
        ...
        { 
            ...
            if (!PrefixState)
                ProgramFail(Parser,(char *)"value not expected here");
        ...
        else if (IS_TYPE_TOKEN(Token))
        {
            ...
            if (!PrefixState)
                ProgramFail(Parser,(char *)"type not expected here");
...
void ExpressionParseMacroCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *MacroName, struct MacroDef *MDef)
{
    ...
    { 
        ...
        if (ParamArray == NULL)
            ProgramFail(Parser,(char *)"out of memory");
    }
    else
        ExpressionPushInt(Parser, StackTop, 0);
    ...
    do {
        if (ExpressionParse(Parser, &Param))
        {
            ...
            { 
                if (ArgCount < MDef->NumParams)
                    ...
                    ProgramFail(Parser,(char *)"too many arguments to %s()", MacroName);
            }
        ...
        { 
            ...
            if (!TokenCloseBracket)
                ProgramFail(Parser,(char *)"bad argument");
        }
    ...
    { 
        ...
        if (ArgCount < MDef->NumParams)
            ProgramFail(Parser,(char *)"not enough arguments to '%s'", MacroName);
        
        if (MDef->Body.Pos == NULL)
            ProgramFail(Parser,(char *)"'%s' is undefined", MacroName);
...
void ExpressionParseFunctionCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
    ...
    { 
        ...
        if (FuncValue->Typ->Base != TypeFunction)
            ProgramFail(Parser,(char *)"%t is not a function - can't call", FuncValue->Typ);
            ...
            { 
                ...
                else
                {
                    if (!FuncValue->Val->FuncDef.VarArgs)
                        ProgramFail(Parser,(char *)"too many arguments to %s()", FuncName);
                }
            }
    ...
    { 
        /* run the function */
        if (ArgCount < FuncValue->Val->FuncDef.NumParams)
            ProgramFail(Parser,(char *)"not enough arguments to '%s'", FuncName);
        ...
        { 
            ...
            if (FuncValue->Val->FuncDef.Body.Pos == NULL)
                ProgramFail(Parser,(char *)"'%s' is undefined", FuncName);
            ...
            if (ParseStatement(&FuncParser, TRUE) != ParseResultOk)
                ProgramFail(&FuncParser,(char *)"function body expected");
        
            if (RunIt)
            {
                if (FuncParser.Mode == RunModeRun && FuncValue->Val->FuncDef.ReturnType != &VoidType)
                    ProgramFail(&FuncParser,(char *)"no value returned from a function returning %t", FuncValue->Val->FuncDef.ReturnType);

                else if (FuncParser.Mode == RunModeGoto)
                    ProgramFail(&FuncParser,(char *)"couldn't find goto label '%s'", FuncParser.SearchGotoLabel);
            }
...
long ExpressionParseInt(struct ParseState *Parser)
{
    ...
    { 
        if (!IS_NUMERIC_COERCIBLE(Val))
            ProgramFail(Parser,(char *)"integer value expected instead of %t", Val->Typ);
