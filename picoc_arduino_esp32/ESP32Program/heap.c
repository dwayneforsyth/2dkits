...
void ShowBigList()
{
    ...
    printf((char *)"Heap: bottom=0x%lx 0x%lx-0x%lx, big freelist=", (long)HeapBottom, (long)&HeapMemory[0], (long)&HeapMemory[HEAP_SIZE]);
    for (LPos = FreeListBig; LPos != NULL; LPos = LPos->NextFree)
        printf((char *)"0x%lx:%d ", (long)LPos, LPos->Size);
...
{
#ifdef DEBUG_HEAP
    printf((char *)"HeapUnpopStack(%ld) at 0x%lx\n", (unsigned long)MEM_ALIGN(Size), (unsigned long)HeapStackTop);
#endif
...
{
#ifdef DEBUG_HEAP
    printf((char *)"Adding stack frame at 0x%lx\n", (unsigned long)HeapStackTop);
#endif
...
#ifdef DEBUG_HEAP
        printf((char *)"Popping stack frame back to 0x%lx\n", (unsigned long)HeapStackTop);
#endif
        return TRUE;
    }
    else
...
#ifdef DEBUG_HEAP
        printf((char *)"allocating %d(%d) from bucket", Size, AllocSize);
#endif
...
#ifdef DEBUG_HEAP
               printf((char *)"allocating %d(%d) from freelist, no split (%d)", Size, AllocSize, (*FreeNode)->Size);
#endif
                NewMem = *FreeNode;
                assert((unsigned long)NewMem >= (unsigned long)&HeapMemory[0] && (unsigned char *)NewMem - &HeapMemory[0] < HEAP_SIZE);
...
#ifdef DEBUG_HEAP
                printf((char *)"allocating %d(%d) from freelist, split chunk (%d)", Size, AllocSize, (*FreeNode)->Size);
#endif
...
#ifdef DEBUG_HEAP
        printf((char *)"allocating %d(%d) at bottom of heap (0x%lx-0x%lx)", Size, AllocSize, (long)((char *)HeapBottom - AllocSize), (long)HeapBottom);
#endif
...
#ifdef DEBUG_HEAP
        printf((char *)"freeing %d from bottom of heap\n", MemNode->Size);
#endif
...
#ifdef DEBUG_HEAP
        printf((char *)"freeing %d to bucket\n", MemNode->Size);
#endif
        assert(FreeListBucket[Bucket] == NULL || ((unsigned long)FreeListBucket[Bucket] >= (unsigned long)&HeapMemory[0] && (unsigned char *)FreeListBucket[Bucket] - &HeapMemory[0] < HEAP_SIZE));
...
#ifdef DEBUG_HEAP
        printf((char *)"freeing %lx:%d to freelist\n", (unsigned long)Mem, MemNode->Size);
#endif
