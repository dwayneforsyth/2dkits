...
{
  sPrintInt(OutCh);return;

   /* if (Stream->FilePtr != NULL)
    {
        // output to stdio stream 
        Serial.print(OutCh);//, Stream->FilePtr);
        Stream->CharCount++;
    }
...
void StdioOutPuts(const char *Str, StdOutStream *Stream)
{
 ...
 /*   if (Stream->FilePtr != NULL)
    {
        /* output to stdio stream */
        Serial.print(Str); //fputs(Str, Stream->FilePtr);
    }
    else
    {
        /* output to a string */
...
void StdioFprintfWord(StdOutStream *Stream, const char *Format, unsigned int Value)
{
    char bf[351];
    {
        snprintf((char *)&bf,350, Format, Value);
        send((char *)&bf);return;
    }
    /*
        ...
        if (*FPos == '%')
        {
            ...
            do
            {
                ...
                if (ShowType == &VoidType)
                {
                    switch (*FPos)
                    {
                        case 'm':   //Ssend(strerror(errno));
                        break;
                        case '%':   //Ssend(*FPos);
                        break;
                        case '\0':  OneFormatBuf[OneFormatCount] = '\0'; //Ssend(*FPos);
                        break;
                        case 'n':   
                ...
                else
                {
                    ...
                    if (ShowType == &IntType)
                    {
                        ...
                        if (IS_NUMERIC_COERCIBLE(ThisArg))
                            StdioFprintfWord(1, OneFormatBuf, ExpressionCoerceUnsignedInteger(ThisArg));
                        else
                            send(F("XXX"));
                    }
#ifndef NO_FP
                    else if (ShowType == &FPType)
                    {
                        ...
                        if (IS_NUMERIC_COERCIBLE(ThisArg))
                            StdioFprintfFP(Stream, OneFormatBuf, ExpressionCoerceFP(ThisArg));
                        else
                            send(F("XXX"));
                    }                    
#endif
                    else if (ShowType == CharPtrType)
                    {
                        if (ThisArg->Typ->Base == TypePointer)
                            StdioFprintfPointer(Stream, OneFormatBuf, ThisArg->Val->Pointer);
                            
                        else if (ThisArg->Typ->Base == TypeArray && ThisArg->Typ->FromType->Base == TypeChar)
                            StdioFprintfPointer(Stream, OneFormatBuf, &ThisArg->Val->ArrayMem[0]);
...
int StdioBaseScanf(struct ParseState *Parser, FILE *Stream, char *StrIn, char *Format, struct StdVararg *Args)
{
    ...
    if (Args->NumArgs > MAX_SCANF_ARGS)
        ProgramFail(Parser,(char *)"too many arguments to scanf() - %d max", MAX_SCANF_ARGS);
    ...
    for (ArgCount = 0; ArgCount < Args->NumArgs; ArgCount++)
    {
        ...
        if (ThisArg->Typ->Base == TypePointer) 
            ...
            ProgramFail(Parser,(char *)"non-pointer argument to scanf() - argument %d after format", ArgCount+1);
    }
...
{
    ReturnValue->Val->Integer = putchar(Param[0]->Val->Integer);// , Param[1]->Val->Pointer);
}
...
struct LibraryFunction StdioFunctions[] =
{
  /*
    ...
    { StdioFreopen,(char *)"FILE *freopen(char *, char *, FILE *);" },
...
void StdioSetupFunc(void)
{
    ...
    StructFileType = TypeCreateOpaqueStruct(NULL, TableStrRegister((char *)"__FILEStruct"), sizeof(FILE));
    ...
    TypeCreateOpaqueStruct(NULL, TableStrRegister((char *)"__va_listStruct"), sizeof(FILE));
    
    /* define EOF equal to the system EOF */
    VariableDefinePlatformVar(NULL,(char *)"EOF", &IntType, (union AnyValue *)&EOFValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"SEEK_SET", &IntType, (union AnyValue *)&SEEK_SETValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"SEEK_CUR", &IntType, (union AnyValue *)&SEEK_CURValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"SEEK_END", &IntType, (union AnyValue *)&SEEK_ENDValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"BUFSIZ", &IntType, (union AnyValue *)&BUFSIZValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"FILENAME_MAX", &IntType, (union AnyValue *)&FILENAME_MAXValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"_IOFBF", &IntType, (union AnyValue *)&_IOFBFValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"_IOLBF", &IntType, (union AnyValue *)&_IOLBFValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"_IONBF", &IntType, (union AnyValue *)&_IONBFValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"L_tmpnam", &IntType, (union AnyValue *)&L_tmpnamValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"GETS_MAX", &IntType, (union AnyValue *)&GETS_MAXValue, FALSE);
    
    /* define stdin, stdout and stderr */
    VariableDefinePlatformVar(NULL,(char *)"stdin", FilePtrType, (union AnyValue *)&stdinValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"stdout", FilePtrType, (union AnyValue *)&stdoutValue, FALSE);
    VariableDefinePlatformVar(NULL,(char *)"stderr", FilePtrType, (union AnyValue *)&stderrValue, FALSE);

    /* define NULL, TRUE and FALSE */
    if (!VariableDefined(TableStrRegister((char *)"NULL")))
...
{
  char bf[3];
  snprintf((char *)&bf,2,(char *)"%c",OutCh);bf[1]=0;
  sprint((char *)&Bf);
