...
struct ValueType *TypeGetMatching(struct ParseState *Parser, struct ValueType *ParentType, enum BaseType Base, int ArraySize, const char *Identifier, int AllowDuplicates)
{
    ...
    if (ThisType != NULL)
    {
        if (AllowDuplicates)
            ...
            ProgramFail(Parser,(char *)"data type '%s' is already defined", Identifier);
    }
...
void TypeParseStruct(struct ParseState *Parser, struct ValueType **Typ, int IsStruct)
{
    ...
    *Typ = TypeGetMatching(Parser, &UberType, IsStruct ? TypeStruct : TypeUnion, 0, StructIdentifier, Token != TokenLeftBrace);
    ...
    { 
        ...
        if ((*Typ)->Members == NULL)
            ProgramFail(Parser,(char *)"structure '%s' isn't defined", LexValue->Val->Identifier);
            
        return;
    }
    
    if (TopStackFrame != NULL)
        ProgramFail(Parser,(char *)"struct/union definitions can only be globals");
    ...
    do {
        ...
        if (MemberType == NULL || MemberIdentifier == NULL)
            ProgramFail(Parser,(char *)"invalid type in struct");
        ...
        if (!TableSet((*Typ)->Members, MemberIdentifier, MemberValue, Parser->FileName, Parser->Line, Parser->CharacterPos))
            ProgramFail(Parser,(char *)"member '%s' already defined", &MemberIdentifier);
            
        if (LexGetToken(Parser, NULL, TRUE) != TokenSemicolon)
            ProgramFail(Parser,(char *)"semicolon expected");
...
//    struct ValueType *EnumType;
    ...
    *Typ = &IntType;
    ...
    { 
        ...
        if ((*Typ)->Members == NULL)
            ProgramFail(Parser,(char *)"enum '%s' isn't defined", EnumIdentifier);
            
        return;
    }
    
    if (TopStackFrame != NULL)
        ProgramFail(Parser,(char *)"enum definitions can only be globals");
    ...
    *Typ = NULL;
    ...
    switch (Token)
    {
        ...
        case TokenStructType: case TokenUnionType: 
            if (*Typ != NULL)
                ProgramFail(Parser,(char *)"bad type declaration");
