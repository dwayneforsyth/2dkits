...
#ifdef TIXCLOCK
#define URL "http://www.2dkits.com/kits/kit55/%02d.%02d.%02d%s/%s"
#else
#define URL "http://www.2dkits.com/kits/kit25/%02d.%02d.%02d%s/%s"
#endif
void auditDiskFiles( void* pvParameters ) {
    ...
    for (int8_t i=0; i < neededFileCount; i++) {
        if (files[i].status != 1) {
            asprintf( &url, URL, MAJOR, MINOR, BUILD, (getSystemType())?"_test":"", files[i].name); 
